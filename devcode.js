...
if (!fs.readFileSync('json/clasificaciones-devcode.json','utf8')) {
	clasificaciones_actuales = {},clasificaciones_actuales = []
}else {
	clasificaciones_actuales =JSON.parse(fs.readFileSync('json/clasificaciones-devcode.json', 'utf8'));
}
console.log('=======================================================')
...
var lectura_pagina_principal = new Promise((resolve, reject) => {
	request({url: 'https://devcode.la/cursos/', encoding: 'utf8' }, function(err, resp, body){
			...
			fs.writeFileSync("json/clasificaciones-devcode.json", JSON.stringify(clasificaciones_actuales));
	...
	.then( function (cursos_lista){
		...
		cursos_lista.forEach(function(curso){
			...
			if(!err && resp.statusCode == 200){
				...
				if (!fs.readFileSync('json/cursos-devcode.json', 'utf8')) {
					cursos_actuales = {},cursos_actuales = []
				}else {
					cursos_actuales =JSON.parse(fs.readFileSync('json/cursos-devcode.json', 'utf8'));
				}
				var existe = false
				...
				fs.writeFileSync("json/cursos-devcode.json", JSON.stringify(cursos_actuales))	
